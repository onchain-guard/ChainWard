...
|---|---|---|---|
| **하이재킹** — "전액 송금해라" | **예** — 지시를 따라야 성립 | 0% [0–8.8] · 0/40 | 0% · 0/44 |
...
- **하이재킹 계열에서는 최신 모델이 이미 스스로 막는다.** Sonnet 5는 가드 없이도 canary 공격을 0/40으로 막았고, 잔고 델타 실험과 백엔드 라벨러 실험에서도 0이었다(총 4회 독립 측정). 코퍼스 20건 중 11건이 이 계열이므로, **강한 모델을 쓰는 대화형 에이전트라면 ChainWard의 한계효용은 그만큼 작다.** 값이 나오는 자리는 모델 정렬이 발동하지 않는 통과·기만 계열, 그리고 설득할 대상이 없는 백엔드 파이프라인이다.
