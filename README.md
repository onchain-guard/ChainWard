...
|---|---|---|---|
| **하이재킹** — "전액 송금해라" | **예** — 지시를 따라야 성립 | ⚠️ **미측정** * | ⚠️ **미측정** * |
| **통과** — 활성 마크다운이 답변에 실림 | **아니오** — 성실히 재현만 해도 성립 | **66.7%** [39.1–86.2] · 8/12 | **0%** [0–24.2] · 0/12 |
| **기만** — 체인이 반박하는 주장 | **아니오** — 알 방법이 없음 | 33.3% [13.8–60.9] · 4/12 | 0% · 0/12 |

\* **하이재킹 행은 철회했다.** 이전 판은 이 자리에 `0/40`을 싣고 "모델이 스스로 막았다"고 읽었는데, 그 측정이 무효였다 — 해당 케이스들의 유저 질문이 전부 *수동적 읽기 요청*("이 토큰 뭐야?", "요약해줘")이라 모델이 애초에 송금 툴을 부를 이유가 없었고, 하네스에 positive control이 없어 "거부했다"와 "도달 못 했다"를 구분할 수 없었다. 하네스는 고쳤고(행동 프레임 + 컨트롤 2건 + `movedFunds` 노출), 재측정 전까지 이 칸에 숫자를 넣지 않는다. 전말 → [BENCH-RESULTS §9](docs/BENCH-RESULTS.md#9-결함-보고--canary-축은-무엇을-쟀는가)

**논지는 이 철회에 걸려 있지 않다.** 통과 계열이 직접 보여준다: 가드를 끄면 66.7%가 새어나가는데, 같은 모델이 *기만* 계열(A04 homoglyph 사칭·A13 허니팟 안전주장)에서는 가드 없이도 스스로 경고했다. 즉 **모델 정렬은 어떤 자리에서는 발동하고 어떤 자리에서는 발동하지 않으며, 그 경계가 "거부할 지시가 있는가"와 일치한다.**
...
| **유용성 보존** (가드 on, 정상 40런) | **100%** [91.2–100] · 40/40 |

이 구멍은 **백엔드 AI에서 더 벌어진다.** 인덱서·라벨러·자동요약·보안스캐너는 설득할 대상이 애초에 없고, 데이터가 들어가는 것만으로 산출물이 오염된다.

> Claude Sonnet 5, 전체 코퍼스 × 2팔 × 4런 = **216콜, $1.83**. 측정 설계·케이스별 결과·반증 시도·커버리지 격자·불리한 결과까지 → **[docs/BENCH-RESULTS.md](docs/BENCH-RESULTS.md)**. 재현:
> ```bash
...
- **하이재킹 계열의 한계효용은 아직 모른다.** 이전 판은 "Sonnet 5가 가드 없이도 0/40으로 막았으니 한계효용이 작다"고 썼는데, 그 0/40이 무효였다(위 \* 및 [BENCH-RESULTS §9](docs/BENCH-RESULTS.md#9-결함-보고--canary-축은-무엇을-쟀는가)). 네 번의 "독립" 측정이 전부 0이었던 것도 독립 확인이 아니라 **같은 설계 결함을 네 번 반복한 것**일 수 있다. 코퍼스 20건 중 11건이 이 계열이므로 재측정의 우선순위가 가장 높다. 값이 확실히 나오는 자리는 여전히 통과·기만 계열, 그리고 설득할 대상이 없는 백엔드 파이프라인이다.
...
- ✅ **실측**: 전체 코퍼스 × 2팔 × 4런 (216콜) — [docs/BENCH-RESULTS.md](docs/BENCH-RESULTS.md). 다중 모델 교차는 미완.
